...
void printInOrderLeaves(Node *root);
...
void printInOrderLeaves(Node *root) {
    if(isTreeEmpty(root)) return;
    printInOrderLeaves(root->left);
    if(isTreeEmpty(root->left) && isTreeEmpty(root->right)) printf("%d ", root->key);
    printInOrderLeaves(root->right);
}
